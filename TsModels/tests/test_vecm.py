...
class TestPrivateHelpers:
    ...
    covers: code/python/Ts/TsModels/_vecm.py::_compute_equation_stats [function]
    covers: code/python/Ts/TsModels/_vecm.py::_add_coef_row [function]
    """
    ...
    def test_compute_equation_stats(self, _dummy):
        ...
        covers: code/python/Ts/TsModels/_vecm.py::_compute_equation_stats [function]
        """
        from Ts.TsModels._vecm import _compute_equation_stats
        import numpy as np

        resid = np.random.randn(100)
        fitted = np.random.randn(100)
        stats = _compute_equation_stats(resid, fitted, 6, 100)
        assert "rmse" in stats
        assert "r_squared" in stats
        assert "chi2" in stats
        assert "p_chi2" in stats
        assert stats["rmse"] > 0

    def test_add_coef_row(self, _dummy):
        """_add_coef_row appends a formatted string to lines.

        covers: code/python/Ts/TsModels/_vecm.py::_add_coef_row [function]
        """
        from Ts.TsModels._vecm import _add_coef_row
        from scipy import stats as scipy_stats

        lines = []
        _add_coef_row(lines, "L1.", 1.5, 0.3, scipy_stats.norm.ppf(0.975))
        assert len(lines) == 1
