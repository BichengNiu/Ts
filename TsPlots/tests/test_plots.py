...
class TestPlotSeries:
    ...
    def test_grid(self):
        data = np.random.randn(20)
        fig, ax = plot_series(data, grid=True)
        # grid should be visible
        plt.close(fig)

    def test_vlines_and_shade(self):
        data = np.random.randn(30)
        fig, ax = plot_series(data, vlines=15, shade=(10, 20))
        plt.close(fig)
...
class TestPlotScatter:
    ...
    def test_with_fit_line(self):
        fig, ax = plot_scatter(x=np.random.randn(30), y=np.random.randn(30), fit_line=True)
        plt.close(fig)
...
class TestPlotPACF:
    def test_returns_fig_ax(self):
        data = np.random.randn(100)
        fig, ax = plot_pacf(data, nlags=10)
        assert isinstance(fig, plt.Figure)
        plt.close(fig)
