"""SARIMA/SARIMAX process simulation and result containers.

Provides :func:`simulate_sarima` for generating synthetic time series from
SARIMA(p,d,q)(P,D,Q,s) models, :func:`simulate_sarimax` for adding a fitted
deterministic response path, and :class:`SimSARIMAResult` for storing and
...
def _normalise_deterministic_path(deterministic, n: int) -> np.ndarray | None:
    """Validate a deterministic SARIMAX response path."""
    if deterministic is None:
        return None
    try:
        values = np.asarray(deterministic, dtype=float)
    except (TypeError, ValueError) as exc:
        raise TypeError("deterministic must be a numeric one-dimensional path") from exc
    if values.ndim != 1 or len(values) != n:
        raise ValueError(
            f"deterministic must contain exactly {n} observations, got shape {values.shape}"
        )
    if not np.all(np.isfinite(values)):
        raise ValueError("deterministic must contain only finite values")
    return values.copy()
...
def simulate_sarimax(
    ...
    """Simulate a SARIMAX response from a deterministic path and SARIMA errors.

    The generated response is ``y_t = m_t + u_t``, where ``m_t`` is the
    supplied deterministic response path and ``u_t`` is generated by the
    SARIMA process.  This matches a SARIMAX model whose static exogenous and
    trend terms are represented by ``m_t``.  The deterministic path is added
    after inverse differencing, so it is not incorrectly integrated with the
    ...
    order : tuple
        ``(p, d, q)`` non-seasonal order for the SARIMA error process.
    seasonal_order : tuple
        ``(P, D, Q, s)`` seasonal order for the SARIMA error process.
    ar : float or list of float, optional
        Non-seasonal AR coefficients for the error process.
    ma : float or list of float, optional
        Non-seasonal MA coefficients for the error process.
    seasonal_ar : float or list of float, optional
    ...
    seed : int, optional
        Random seed for reproducibility.
    burn : int
        Number of stochastic burn-in observations to discard.

    Returns
    -------
    SimSARIMAResult
        Simulation result containing the full SARIMAX response and the
        underlying SARIMA innovations.

    Raises
    ------
    TypeError
        If ``deterministic`` cannot be converted to a numeric path.
    ValueError
        If ``deterministic`` is not finite or does not have length ``n``.
    ...
    ...     n=30, order=(1, 0, 0), ar=[0.6],
    ...     deterministic=np.full(30, 2.0), seed=42,
    ... )
    >>> result.data.shape
    (30,)
    """
