"""Shared plotting helper for unit root test results.

Used by ADF, Phillips-Perron, KPSS, Perron, and Zivot-Andrews result
classes to render diagnostic charts.
"""
...
from Ts.TsPlots.style import (
    DEFAULT_PALETTE, FIGSIZE, LEGEND_FONTSIZE, style_axes,
)
...
def _render_critical_value_plot(result, test_name, ax=None):
    ...
    if not crit:
        ax.text(0.5, 0.5, "No critical values available", ha="center",
    ...
    for i, (lab, cv) in enumerate(zip(labels, crit_vals)):
        color = colors[i % len(colors)]
        ax.axhline(
            cv, color=color, linestyle="--", linewidth=1.5 if i == 1 else 1.0,
            alpha=0.8, label=f"{lab} critical value: {cv:.3f}",
        )

    # Test statistic as a scatter point
    ax.scatter(
        0, result.statistic, color=DEFAULT_PALETTE[4], s=120,
        zorder=5, label=f"Test statistic: {result.statistic:.3f}",
    )
...
def _render_tstat_plot(result, ax=None):
    ...
    ax.plot(years, t_stats_arr, color=DEFAULT_PALETTE[0], linewidth=2,
            marker="o", markersize=4, markerfacecolor="white",
            markeredgecolor=DEFAULT_PALETTE[0], markeredgewidth=1.5,
            label="t(rho-hat)")

    # Highlight the minimum
    min_idx = np.argmin(t_stats_arr)
    ax.axvline(years[min_idx], color=DEFAULT_PALETTE[4], linestyle="--",
               linewidth=1.5, alpha=0.7,
               label=f"Optimal break: {years[min_idx]:.0f}")

    # Critical value lines
    ax.axhline(result.cv_05, color=DEFAULT_PALETTE[2], linestyle=":",
               linewidth=1.5, alpha=0.8,
               label=f"5% critical value ({result.cv_05:.2f})")
    ax.axhline(result.cv_01, color=DEFAULT_PALETTE[3], linestyle=":",
               linewidth=1.5, alpha=0.8,
...
def _render_ic_plot(result, ax=None):
    ...
    ax.plot(valid_ks, valid_ic, color=DEFAULT_PALETTE[0], linewidth=2,
            marker="o", markersize=6, markerfacecolor="white",
            markeredgecolor=DEFAULT_PALETTE[0], markeredgewidth=1.5)

    # Highlight minimum
    ax.axvline(best_k, color=DEFAULT_PALETTE[4], linestyle="--",
               linewidth=1.5, alpha=0.7,
