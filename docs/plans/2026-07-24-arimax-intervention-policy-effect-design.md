...
事件映射以“完整建模日历”为基准，再投影到当前拟合或预测切片，而不是把当前切片误当成完整时间轴。这样，早于预测切片的 `step` 会正确延续，晚于训练切片但落在未来日历中的事件也不会被错误映射回训练期。事件在当前日历覆盖区间外时暂不贡献；日历扩展到该日期后再按规则映射。若事件落在完整日历覆盖区间内却无法按指定规则映射、无法推断 `period` 所需频率或时区不一致，则明确失败。拟合期最终生成的全零事件列仍因不可识别而失败，不静默删除。多个事件日期合法映射到同一期时，按上述计数语义编码。
